const o=`---
title: Favourites and low stock
...
Two small features make a big inventory easier to manage: favourites let you pin the spools you reach for most, and low-stock warnings flag the ones about to run out.

## Favourites

Tap the star on any spool card (or the heart on the spool detail screen) to favourite it. The flag is the same either way — tap again to remove it.

Favourites surface in a few places:

- A **Favorites** filter chip in the filter sheet, to show only the spools you have pinned.
- A favourites count on the Stats tab.
- A custom drag order, when you want to arrange your pinned spools by hand.

### Rearranging favourites by hand

You can drag favourited spools into your own order when all of these are true: the **Favorites** filter is on, the list is **Flat** (not grouped), you are not in compact list mode, and selection mode is off. A **Drag to reorder favorites** hint appears when this is available. Your custom order is remembered between sessions.

In any other mode — grouped, sorted by something other than date added, or on the compact list — favourites follow the sort you have chosen.
