...
NFC scanning works on iPhone and Android only. There is no NFC button on Windows, macOS, Linux, or Web — the hardware is not there.
...
On Android, open the system **Settings → Connected devices → NFC** (the path varies by manufacturer) and make sure NFC is switched on. A dead battery-saver or a thick metal case can block reads.
...
A plain blank tag, a hotel key, a payment card, or any tag in an unknown format produces **Unrecognized NFC tag format.** A tag Spool Hoarder recognises but cannot read cleanly produces **Could not parse tag data.**
...
| Message | Cause | Fix |
|---|---|---|
| **NFC scan cancelled** | You pulled the phone away or the system session timed out. | Hold the phone steady on the tag for the whole read. |
...
NFC antennas live near the top back of most phones, not in the centre. Hold that spot flat against the tag.
