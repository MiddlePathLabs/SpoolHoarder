...
description: Suggestions for filaments that pair well with a colour you already own.
order: 3
status: published
updated: 2026-08-04
keywords:
...
The Colour Advisor looks at the spool you are viewing and suggests other filaments
from your inventory whose colours pair well with it, using classic colour theory.
It is a planning aid for multi-colour prints, not a scanner or an inventory rule.
...
Advisor Suggestions** if you would rather pick colours yourself.

## What drives a suggestion

The advisor resolves the base spool's colour — most accurate when the spool has a
hex code, and still workable from its colour family or name — then scores every
...
Visually identical colours are deduplicated, so a strip shows distinct options.

Each palette shows the in-stock spools that fill it, and, for the colours you are
...
When a colour was inferred from the spool's name rather than a hex code, the chip
notes that the exact shade may vary.

## What the palette names mean

Each suggestion belongs to one colour-theory palette:

- **Complementary** — the colour opposite on the wheel, for bold contrast.
- **Analogous** — neighbouring shades that blend naturally.
...
- **Monochromatic** — the same colour in lighter and darker tones.
- **Square tetradic** and **Rectangle tetradic** — four-hue palettes built from
  two complementary pairs.

For a neutral base (black, white, or grey) there is no hue to harmonise with, so
...
- **The base spool has no usable colour data.** Add a hex code, a colour family,
  or a descriptive colour name and suggestions return.
...
- **No colour lands close enough to a harmony target.** The advisor would rather
