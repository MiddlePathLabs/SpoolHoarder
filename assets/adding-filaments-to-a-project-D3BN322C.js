...
The filaments on a project are your plan: the spools, colours, and amounts a
print is meant to use. They are separate from the usage you log afterwards, so
...
identical in brand, material, colour, and colour code are shown as a single
entry, so a full shelf of the same filament does not clutter the list. Use the
**Search** box to filter by brand, material, or colour.
...
Leave the required weight blank if you only want to record what a colour is, not
how much it should use.

## Multi-colour and multi-material prints

Add one filament entry for each colour or material in the print. A two-colour
...
## Where colour suggestions come from

Once the project has at least one filament, the picker shows a **Suggestions**
drawer with complementary colours. These come from two places:

- The [Color advisor](/docs/planning/color-advisor/), which suggests pairings
  based on colour theory and the filaments already on the project.
