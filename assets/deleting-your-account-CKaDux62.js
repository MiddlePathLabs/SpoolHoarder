...
**Deleting your account does not cancel a subscription.** Pro is billed by the App Store or the Microsoft Store, not by Spool Hoarder, so a subscription keeps renewing and charging you after the account is gone. Cancel it in the store first, then come back and delete the account. [Managing or cancelling](/docs/account/managing-or-cancelling/) shows where to do that on each platform.
...
4. Type **DELETE** in the **Type DELETE to confirm** field. The red **Delete Account** button stays greyed out until you do.
...
5. Confirm. Everywhere except the web app, Spool Hoarder first tries to take a safety backup on the device; if that fails, it warns you and asks whether to clear anyway. You then get an **All data cleared** confirmation.
...
Clearing local data does not delete anything in the cloud, and it does not delete your account. If you are signed in with sync on, the cleared records are not downloaded again afterwards.
